...
/* SUPERPOSITION 2.0 bake — one pure-template implementation consumed by browser + Node.
   CSS source is split only to keep repository transfer/maintenance bounded; it bakes as one sheet. */
...
const sections={'view-home':homeHTML(data.identity),'view-record':recordHTML(data.record),header:headerHTML(data.identity),'domain-strip':domainStripHTML(data),index:indexHTML(data),footer:footerHTML(data.identity)};
...
const cssFiles=['css/main.css','css/main.1.css','css/main.2.css','css/main.3.css'];
const css=cssFiles.map(f=>readFileSync(join(ROOT,f),'utf8')).join('').trim().replace(/url\("\.\.\//g,'url("');
const cssRe=/(\/\*BAKE:css BEGIN\*\/)[\s\S]*?(\/\*BAKE:css END\*\/)/;if(!cssRe.test(html))missing.push('css');else html=html.replace(cssRe,`$1\n${css}\n$2`);
