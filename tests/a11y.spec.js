...
test.describe("a11y",()=>{
  for(const route of ["/?seed=1&force=static","/?seed=1&force=static#view-columns","/?seed=1&force=static#view-clusters"]){
  ...
  test("axe: zero critical on standalone resume",async({page})=>{await page.goto("/resume/");const r=await scan(page);expect(critical(r).map(v=>v.id)).toEqual([]);});
  test("axe: zero critical with dossier and Index",async({page})=>{
    await page.goto("/?seed=1&force=static#view-columns");await page.locator('#view-columns .dossier > button').first().click();let r=await scan(page);expect(critical(r).map(v=>v.id)).toEqual([]);
  ...
  test("dossier aria-controls resolves",async({page})=>{await page.goto("/?force=static#view-columns");const btn=page.locator('#view-columns .dossier > button').first();const controls=await btn.getAttribute('aria-controls');const region=page.locator(`#${controls}`);await expect(region).toHaveAttribute('role','region');await expect(region).toHaveAttribute('aria-labelledby',await btn.getAttribute('id'));});
});
